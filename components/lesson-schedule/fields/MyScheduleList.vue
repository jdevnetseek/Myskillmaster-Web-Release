<template>
  <div
    class="my-schedule-list"
    :class="{
      'screen-border': hasScreenMargin,
    }"
  >
    <div
      class="my-schedule-list__header d-flex justify-space-between align-center mb-3"
    >
      <h5>{{ header }}</h5>
    </div>
    <v-row
      ...
      class="my-schedule-list__list"
    >
      <template v-if="list.length > 0">
        <v-col v-for="(schedule, index) in list" :key="index" cols="12" md="6">
          <LessonCard
            :orientation="landscape"
            :lesson="schedule"
            :type="type"
            :has-rating="false"
            class="my-schedule-list__list__item"
            @click="
              $router.push({
      ...
      <template v-else-if="!form.$busy">
        <div class="my-schedule-list__list__placeholder">
          <p class="mb-0">
            No lessons found
          </p>
        </div>
...
import { SCHEDULE } from '@/utils/enums/LessonCardType'
...
export default {
  ...
  computed: {
    ...mapState({
      form: state => state.lessonSchedule.listing,
      list: state => state.lessonSchedule.list,
      meta: state => state.lessonSchedule.listMeta
    }),
  ...
  methods: {
    ...
    async fetch(page = null, initial = false) {
      ...
      await this.$store.dispatch('lessonSchedule/getList', params).then(() => {
