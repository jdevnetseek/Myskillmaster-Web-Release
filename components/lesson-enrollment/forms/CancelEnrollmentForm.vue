<template>
  <v-form class="cancel-enrollment-form" @submit.prevent="submit">
    <div class="cancel-enrollment-form__header">
    ...
    <div class="cancel-enrollment-form__fields">
      <v-row>
    ...
    <div class="cancel-enrollment-form__actions">
      <v-row>
        <v-col cols="6">
          <v-btn
            color="#E6F1F1"
            class="cancel-enrollment-form__btn"
            depressed
            align="right"
            :disabled="form.$busy"
            :loading="form.$busy"
...
/**
 * ==================================================================================
 * Cancel lesson enrollment form
 * ==================================================================================
 **/

import { mapState } from 'vuex'
import { hasError, validationMixin } from '@/validations'
import { cancelEnrollmentForm } from '@/validations/lessonEnrollment/cancelEnrollment'
...
export default {
  ...
  computed: {
    hasError,
    ...mapState({
      form: state => state.lessonEnrollment.enrollmentCancel,
  ...
  methods: {
    ...
    async submit(e) {
      ...
      if (!this.form.$busy && !this.$v.$invalid) {
        await this.$store
          .dispatch('lessonEnrollment/cancelEnrollment', {
          ...
          .then((enrollment) => {
            if (this.form.$hasErrors()) {
              ...
              this.reset()
              this.showSnackbar('Successfully cancelled your enrollment')
              this.$router.push({
                name: 'my-lessons'
              })
            }
